...
export function initializeGameSocket(server, sessionMiddleware) {
  ...
  io.on('connection', (socket) => {
    ...
    // Player updates role
    socket.on('update_role', async ({ roomId, playerId, role }) => {
      try {
    ...
    socket.on(
      'kick_player',
      async ({ roomId, playerId: playerToKick, adminId }) => {
        try {
          const room = await Room.findByRoomId(roomId);
          ...
          const kickedPlayer = room.players.find(
            (p) => p.playerId === playerToKick
          );
          if (!kickedPlayer) return;

          await Room.removePlayer(roomId, playerToKick);
          const updatedRoom = await Room.findByRoomId(roomId);

          // Clear kicked player's session
          const allSockets = await io.fetchSockets();
          for (const s of allSockets) {
            if (s.playerId === playerToKick && s.roomId === roomId) {
    ...
    // Request target word from wordmaster
    socket.on('request_target_word', async ({ roomId, wordmasterId }) => {
      try {
        const room = await Room.findByRoomId(roomId);
        const wordmaster = room.players.find(
          (p) => p.playerId === wordmasterId
        );

        // Update room status to show game is starting
        await Room.updateStatus(roomId, 'starting');

        // Notify everyone that wordmaster is choosing (with wordmaster's nickname)
        io.to(roomId).emit('wordmaster_choosing', {
          wordmasterId,
          wordmasterNickname: wordmaster.nickname,
        });

        // Notify wordmaster to set target word
        io.to(roomId).emit('show_target_word_modal', { wordmasterId });
      } catch (error) {
        console.error('Error requesting target word:', error);
        socket.emit('error', { message: 'Failed to request target word' });
      }
    });
    ...
    socket.on(
      'start_game',
      async ({ roomId, wordmasterId, targetWord, wordType }) => {
        try {
          const room = await Room.findByRoomId(roomId);
          if (!room) {
          ...
          if (!wordmaster || guessers.length < 2) {
            socket.emit('error', {
              message: 'Need 1 wordmaster and at least 2 guessers',
            });
            return;
          }
          ...
          io.to(roomId).emit('game_started', firstRound);
        } catch (error) {
